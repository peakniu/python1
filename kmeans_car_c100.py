#-*- coding: utf-8 -*-
#使用K-Means算法聚类家自车投保商业险的保单中投保百万三者的保单分析
import pandas as pd
import numpy as np
...
data = pd.read_sql('SELECT * FROM nzz_kmeans_100', engine)#投保百万三者分析
...
data_zs = 1.0*(data - data.min())/(data.max()-data.min()) #数据标准化
print(data_zs)
...
pic_output = './pd_kmeans_100c' #概率密度图文件名前缀
for i in range(k):
    plt.figure(num=i)
